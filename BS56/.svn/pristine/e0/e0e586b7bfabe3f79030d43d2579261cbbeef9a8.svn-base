...
function openNew(){
	...
	$('#add-dlg').dialog('open').dialog('setTitle','车组暂存新增');
	$('#ctype').val("20");
	$('#status').val("10");
	$("#routecode1").combobox({
	    url : baseURL+"/comm/combobox/getRoutesComboboxByDeptid.json?deptid=",//返回json数据的url
	...
	$("#reasonid").combobox({
		url:baseURL+"/sys/basetypeInfo/getBasetypeInfoByTypecode.json?typecode=TEMPREASON", //数据来源
	...
	initOrderTable();
	//alert($('#orderDataTable').datagrid('getRows'));
	$('#orderDataTable').datagrid('loadData', { total: 0, rows: [] });
	//alert($('#orderDataTable').datagrid('getRows'));
}
...
function initOrderTable(){
	  	...
	  	$('#add-fm').form('submit',{
	  		onSubmit: function(){
	  			...
	  			if(isValidate){
	  				...
	  				if(reasonid==""){
	  					$.messager.alert('提示',"请选择车组暂存原因",'info');
	  					return false;
	  				}
	  				var quantity=$('#quantity').numberbox("getValue");
	  				if(quantity==""){
	  					$.messager.alert('提示',"请选择要暂存的订单",'info');
	  					return false;
	  				}
	  			}
	  			return isValidate;
			...
			beforeSend : function () {
	  		...
	  		success: function(data){
	    		...
	    		$.messager.show({
					title : '提示',
					msg :  data.total+'个车组暂存新增'+data.msg+'！',
				});
				//$('#loading').window('close');
			}
	  	});
	...
	function delOperate(){
		...
		if(rows.length==0){
			$.messager.alert('提示',"请选择要删除的暂存信息",'info');
			return;
		}
		$.messager.confirm('提示','确定要删除所选暂存信息吗?',function(result){
			if (result){
				var ids = "";
				...
				$.post(baseURL+'/account/operate/doDelOperate.json'+ids,function(data){
					...
					$.messager.show({
						title : '提示',
						msg :  data.total+'个暂存信息删除'+data.msg+'！',
					});
				});
			}
		})
