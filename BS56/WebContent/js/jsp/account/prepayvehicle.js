...
jQuery(function($){
	...
	$('#dataTabel').datagrid({
		...
		url:baseURL+"/account/prepay/selectShiporderPerpayList.json", //数据来源
		sortName: 'id', //排序的列
		sortOrder: 'desc', //倒序
		...
		columns:[[
			...
			{field:'customername',title:'客户名称',width:$(this).width()*0.15,sortable:true,
	 ...
	 $.messager.confirm('提示','确定要导出到excel吗?',function(result){
	        if (result){
	        	 $('#queryForm').form('submit', {
	        		 url: baseURL+'/account/prepay/doExportToExcel.json?orderdate='+orderdate+"&calcid="+calcid+"&deptid="+deptid
