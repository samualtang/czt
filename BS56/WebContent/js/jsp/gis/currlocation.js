...
jQuery(function($){
	...
	$('#dataTabel').datagrid({
		...
		url:baseURL+"/gis/currlocation/getCurrlocationPageList.json", //数据来源
		sortName: 'id', //排序的列
		sortOrder: 'desc', //倒序
		...
		columns:[[
			...
			{field:'batchId',title:'批次ID',width:$(this).width()*0.2,
				formatter:function(value,row,index){return row.batchId;} //需要formatter一下才能显示正确的数据
			},
			{field:'truckNum',title:'车牌',width:$(this).width()*0.1,
				formatter:function(value,row,index){return row.truckNum;}
			},
			{field:'outDate',title:'发货日期',width:$(this).width()*0.1,
				formatter:function(value,row,index){return row.outDate;} //需要formatter一下才能显示正确的数据
			},
			{field:'currentPos',title:'当前位置',width:$(this).width()*0.1,sortable:true,
				formatter:function(value,row,index){return row.currentPos;}
			},
			{field:'longitude',title:'经度',width:$(this).width()*0.05,
				formatter:function(value,row,index){return row.longitude;}
			},
			{field:'latitude',title:'纬度',width:$(this).width()*0.05,sortable:true,
				formatter:function(value,row,index){return row.latitude;}
			},
			{field:'statusname',title:'状态',width:$(this).width()*0.05,sortable:true,
