...
jQuery(function(def){
	...
	$('#dataTable').datagrid({
		...
		collapsible:true,//可折叠		url:baseURL+"/wms/inout/getOperatePageList.json", //数据来源
		sortName: 'id', //排序的列
	...
	function refund(){
		...
		if(rows.length==0){
		$.messager.alert('提示',"请选择要退货的订单信息",'info');
			return;
		}
		$.messager.confirm('提示','确定要对所选订单进行退货操作吗?',function(result){
			if (result){
				var ids = "",ordernos="";
				$.each(rows,function(i,n){
		      		if(i==0){ 
		      			ids += "?id="+n.id;
		      		    ordernos +="&orderno="+n.orderno;
		      		}else{
		      			ids += "&id="+n.id;
		      			ordernos +="&orderno="+n.orderno;
					}	
		      	});
		      	$.post(baseURL+'/wms/inout/doRefund.json'+ids+ordernos,function(data){
			        	$('#dataTable').datagrid('reload'); 
			        	//console.log("del--"+data);
			        	data = eval('('+data+')');
		      		$.messager.show({
							title : '提示',
							msg :  data.total+'个订单退货'+data.msg+'！',
						});
		      	});
			}
		})
