...
jQuery(function($){
	...
	$('#dataTabel').datagrid({
		...
		url:baseURL+"/sq/starinfo/getStarinfo.json", //数据来源
		sortName: 'id', //排序的列
		...
		columns:[[
			...
			{field:'starname',title:'星级名称',width:20,sortable:true,
				formatter:function(value,row,index){return row.starname;} //需要formatter一下才能显示正确的数据
			},
			{field:'minscore',title:'最小分值',width:20,sortable:true,
				formatter:function(value,row,index){return row.minscore;}
			},
			{field:'maxscore',title:'最大分值',width:30,
				formatter:function(value,row,index){return row.maxscore;}
			},
			{field:'createid',title:'录入人',width:30,
				formatter:function(value,row,index){return row.username;}
			},
			{field:'updatetime',title:'录入时间',width:30,
 ...
 */
function newadd(){
	$('#add-dlg').dialog('open').dialog('setTitle','增加星级参数信息');
	$('#add-fm').form('clear');
}
...
function saveAdd(){
	$('#add-fm').form('submit',{
		...
		url:baseURL+"/sq/starinfo/insertStarinfo.json",
		...
		success: function(data){
    		...
    		$.messager.show({
				title : '提示',
				msg :  data.total+'个星级信息新增'+data.msg+'！',
			});
			//$('#loading').window('close');
		}
	});
 ...
 */
  ...
  function saveEdit(){
		$('#edit-fm').form('submit',{
			...
			url:baseURL+"/sq/starinfo/updateStarinfo.json",
			...
			success: function(data){
	    		...
	    		$.messager.show({
					title : '提示',
					msg :  data.total+'个星级信息已修改'+data.msg+'！',
				});
				//$('#loading').window('close');
			}
		});
...
//删除
	function deleterow(){
		var rows = $('#dataTabel').datagrid('getSelections');
		if(rows.length==0){
		$.messager.alert('提示',"请选择你要删除的星级信息",'info');
		return;
	}
        ...
        if (result){
        	...
        	$.post(baseURL+'/sq/starinfo/delStarinfo.json'+ps,function(data){
	        	$('#dataTabel').datagrid('reload'); 
	        	//console.log("del--"+data);
        		$.messager.show({
					title : '提示',
					msg :  data.total+'个星级信息被删除'+data.msg+'！',
				});
        	});
        }
    });
